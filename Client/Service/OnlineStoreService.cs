...
using OrderControllerPathsOfClientSide = WebAppAssembly.Shared.Entities.Urls.OrderControllerUrls;
...
namespace WebAppAssembly.Client.Service
{
    ...
    public class OnlineStoreService : IOnlineStoreService
    {
        ...
        private IPersonalInfo<OrderClient> _personalInfo;
        private OnlineStoreItem _commonItem;
        private string _tgMainBtnColor;
        ...
        {
            get => _personalInfo;
            private set => _personalInfo = value;
        }

        public OnlineStoreItem CommonItem
        {
            get => _commonItem;
            set => _commonItem = value;
        }

        public Guid? CurrentGroupId { get; private set; }

        public CurrOrderItem? CurrItemPosition { get; private set; }

        public bool IsReleaseMode { get; private set; }

        public string TlgMainBtnColor
        {
            get => _tgMainBtnColor ?? string.Empty;
            private set => _tgMainBtnColor = value;
        }
        ...
        private OnlineStoreService(HttpClient client, OnlineStoreItem commonItem, IPersonalInfo<OrderClient> personalInfo,
            bool isRelease, string tgMainBtnColor)
        {
            _http = client;
            _commonItem = commonItem;
            _personalInfo = personalInfo;
            IsReleaseMode = isRelease;
            _tgMainBtnColor = tgMainBtnColor;
        ...
        public static async Task<OnlineStoreService> CreateAsync(HttpClient client, long chatId, string urlPathOfMainInfo)
        {
            // Retrieves the necessary data for the operation of the online store
            var common = await RetrieveMainInfoForOnlineStoreAsync(client, chatId, urlPathOfMainInfo);

            // Extracts the necessary information for the operation of the online store
            var commonItem = new OnlineStoreItem(common.OnlineStoreItem);

            // Extracts a customer's personal data of the order
            var personalInfo = InitPersonalInfo(common.PersonalOrderInfo);

            // Sets the opeartion mode for the online store
            var isReleaseMode = common.IsReleaseMode;
            ...
            var tlgMainBtnClr = !string.IsNullOrWhiteSpace(common.OnlineStoreItem.TgMainBtnColor)
                ...
                nameof(Exception), $"{typeof(OnlineStoreItem).FullName!}.{nameof(OnlineStoreItem.TgMainBtnColor)}",
                ExceptionType.NullOrEmpty);

            // Sets a color for the Tg's main button
            tlgMainBtnClr = IsRgbColorFormatCorrect(tlgMainBtnClr) ? tlgMainBtnClr : throw new InfoException(typeof(OnlineStoreService).FullName!,
            ...
            return new OnlineStoreService(client, commonItem, personalInfo, isReleaseMode, tlgMainBtnClr);
        }
        ...
        public async Task<bool> TryToCreateInvoiceLinkAsync(IJSRuntime jsRuntime, ITwaNet twaNet)
        {
            try
            {
                ...
                if (!result.Ok)
                {
                    await twaNet.ShowOkPopupMessageAsync(jsRuntime, string.Empty, result.Message ?? string.Empty,
                        HapticFeedBackNotificationType.Error);

                    throw new InfoException(typeof(OnlineStoreService).FullName!, nameof(TryToCreateInvoiceLinkAsync), nameof(Exception),
                        result.Message ?? string.Empty);
                }
                else if (string.IsNullOrEmpty(result.InvoiceLink))
                {
                    // What to send to an user abount this situation ???
                    throw new InfoException(typeof(OnlineStoreService).FullName!, nameof(TryToCreateInvoiceLinkAsync), nameof(Exception),
                        $"{typeof(InvoiceLinkStatus).FullName!}.{nameof(InvoiceLinkStatus.InvoiceLink)}", ExceptionType.NullOrEmpty);
                }
        ...
        public async Task CancelCurrSimilarSelectedItemsWithModifiersAsync()
        {
            ...
            PersonalInfo.Order.RemoveItemsById(product.Id);
            HaveSelectedProductsAtFirst();
        ...
        public async Task SetPickupTerminalWithCalculatingCheckinAsync(IJSRuntime jsRuntime, ITwaNet twaNet, Guid id)
        {
            ...
            PersonalInfo.PersonalItem.SetDeliveryTerminal(pickupTerminal.Id, pickupTerminal.Name);
            ...
            (IJSRuntime, ITwaNet)? jsProcessing = null, Product? product = null, Guid? positionId = null)
        {
            ...
            return tuple switch
            {
                (PageViewType.SelectingProducts, _, Product item, _)
                => await AddOrIncreaseItemPositionForPageWhereProductsAreSelectedAsync(item),
                (PageViewType.ShoppingCart, _, Product item, _)
                => (await IncreaseItemPositionForBasketPageAsync(jsProcessing, item.Id, positionId), null),
                (PageViewType.ChangingSelectedProductsWithModifiers, _, Product item, Guid _positionId)
                => (await IncreaseItemPositionForPageWhereSampleItemPositionAreChangedAsync(item.Id, _positionId), null),
                (PageViewType.SelectingModifiersAndAmountsForProduct, _, _, _)
                => (IncreaseItemPositionForPageWhereModifiersAndSizesOfItemPositionAreSelected(), null),
                (PageViewType.SelectingAmountsForProducts, _, _, _)
                => (IncreaseItemPositionForPageWhereNumberOfItemPositionIsIncreased(), null),
        ...
        public async Task<(IOrderItemProcessing? Item, bool IsBasketEmpty, bool IsSeveralItemPosition)>
            ...
            Product? product = null, Guid? positionId = null)
        {
            var tuple = (pageView, jsProcessing, product, positionId);
            ...
            return tuple switch
            {
                ...
                (PageViewType.ShoppingCart, _, Product item, Guid _positionId)
                => await RemoveOrDecreaseItemPositionForBasketPageAsync(jsProcessing, item.Id, _positionId),
                (PageViewType.ChangingSelectedProductsWithModifiers, _, Product item, Guid _positionId)
                => await RemoveOrDecreaseItemPositionForPageWhereSampleItemPositionAreChangedAsync(item.Id, _positionId),
                (PageViewType.SelectingModifiersAndAmountsForProduct, _, _, _)
                => RemoveOrDecreaseItemPositionForPageWhereModifiersAndSizesOfItemPositionAreSelected(),
                (PageViewType.SelectingAmountsForProducts, _, _, _)
                => RemoveOrDecreaseItemPositionForPageWhereNumberOfItemPositionIsIncreased(),
            ...
            string urlPathOfMainInfo)
        {
            try
            {
                var chatInfo = new TgChat() { ChatId = chatId };
        ...
        private static IPersonalInfo<OrderClient> InitPersonalInfo(PersonalInfo personalInfo)
            => new PersonalInfo<OrderClient>(
                ...
                personalItem: personalInfo.PersonalItem?.ToPersonalItem() ?? new PersonalItem());
            ...
            do
            {
                ...
                if (!checkinResult.Ok || checkinResult.Checkin is null)
                {
                    ...
                    if (IsReleaseMode)
                    {
                        ...
                        var res = await twaNet!.ShowPopupParamsAsync(jsRuntime, popupPrms, HapticFeedBackNotificationType.Warning,
                            HapticFeedbackImpactOccurredType.Soft);

                        switch (res)
                        {
                            case skip:
                                await twaNet.HideProgressAsync(jsRuntime);
                                return new LoyaltyCheckInfoRequest(ok: false)
                                {
                    ...
                    foreach (var loyaltyProgram in checkin.LoyaltyProgramResults)
                    {
                        if (loyaltyProgram.Discounts is not null)
                        {
                            ...
                            foreach (var discount in loyaltyProgram.Discounts)
                            {
                                ...
                                if (discount.Code == OperationTypeCode.FreeProduct && discount.OrderItemId is not null && discount.OrderItemId != Guid.Empty)
                        ...
                        if (loyaltyProgram.FreeProducts is not null)
                        {
                            ...
                            foreach (var freeProduct in loyaltyProgram.FreeProducts)
                            {
                                if (freeProduct.Products is not null)
                                {
                                    ...
                                    {
                                        var sourceProduct = products?.FirstOrDefault(x => x.Id == product.Id);

                                        if (sourceProduct is not null)
                                        {
                                            freeItems.Add(new OrderItem(
                                                sourceProduct.Id,
                                                sourceProduct.Name,
                                                amount: 1,
            ...
            Product product)
        {
            ...
            if (product.HaveModifiersOrSeveralSizes)
            {
                ...
                var item = PersonalInfo.Order.AddItemWithNewPosition(product, product.Sizes.FirstOrDefault()?.SizeId);
            ...
            {
                var item = items.FirstOrDefault(x => x.ProductId == product.Id) ?? PersonalInfo.Order.AddItemWithNewPosition(product);
                IncreaseAmountOfProduct(item);
        ...
        private IOrderItemProcessing IncreaseItemPositionForPageWhereModifiersAndSizesOfItemPositionAreSelected()
        {
            ...
            var currItemInfo = CurrItemPosition ?? throw new InfoException(typeof(OnlineStoreService).FullName!,
                ...
                typeof(CurrOrderItem).FullName!, ExceptionType.Null);
            ...
            RemoveOrDecreaseItemPositionForPageWhereModifiersAndSizesOfItemPositionAreSelected()
        {
            // Gets a current item position of the order
            var currItemInfo = CurrItemPosition ?? throw new InfoException(typeof(OnlineStoreService).FullName!,
                nameof(RemoveOrDecreaseItemPositionForPageWhereModifiersAndSizesOfItemPositionAreSelected), nameof(Exception), typeof(CurrOrderItem).FullName!,
                ExceptionType.Null);
            var item = currItemInfo.GetItem();
        ...
        /// <returns></returns>
        private async Task<(IOrderItemProcessing? Item, bool IsBasketEmpty, bool IsSeveralItemPosition)>
            RemoveOrDecreaseItemPositionForPageWhereProductsAreSelectedAsync(Product product)
        {
            if (PersonalInfo.Order.HaveSampleItemPositions(product.Id))
            {
                // Sets the current product for changing
                CurrItemPosition = new(product);
                return (null, false, true);
            ...
            // Gets an item position
            var item = PersonalInfo.Order.FirstItemByIdOrDefault(product.Id);
            if (item is null) return (null, !PersonalInfo.Order.HaveSelectedItems, false);
